...
def plot_congestion_map_old(congestion, year):
    ...
    fig.update_layout(
        ...
        xaxis_title="Average hourly utilization rate",
        yaxis_title="Average price spread (€/MWh)",
        legend_title="Flow direction",
        xaxis=dict(range=[0, 1]),
        yaxis=dict(range=[0, df["spread_avg_€/MWh"].max() * 1.2])
    )
    return fig
...
def plot_congestion_map(yearly_report, year):
    ...
    df = yearly_report.loc[yearly_report["year"] == year].copy()
    ...
    fig.update_layout(
        title=f"France congestion map ({year})",
        xaxis_title="Average hourly utilization rate",
        yaxis_title="Average hourly price spread (€/MWh)",
        legend_title="Flow direction",
